use std::{sync::mpsc::{self, SyncSender}, thread::{self, JoinHandle}};
        ...
        let handle = thread::spawn(move || {
            for cmd in rx {
                match cmd {
                    ...
                    Command::LogPoints(path, positions) => rec.log(path, &Points3D::new(positions)).expect("Rerun: unable to log points"),
                }
            }
        });
